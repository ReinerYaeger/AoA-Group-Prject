...
public class Person{
    ...
    public boolean isRelatedTo(Person p) {
     ...
     public boolean containSameActivity(Person p) {
        ...
        for (Activity a : activity) {
            for (Activity b : p.getActivity()) {
                if (a.getActivityName().equals(b.getActivityName()))
                    return true;
            }
        }
        return  false;
    ...
    public boolean containSameActivity(Activity ac) {
        //for each item in the activity list check if the other person has the same activity

        for (Activity a : activity) {
            if (a.getActivityName().equals(ac.getActivityName()))
                return true;
        }
        return  false;
    }
